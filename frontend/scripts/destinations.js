...
const toggleButton = document.querySelector('.destination-tabs');
const tabsWrapper = document.querySelector('.left-column .tabs');
...
// Generate Cards for Active Tab
const generateCards = (category) => {
...
// Initial Render
generateCards(Object.keys(data)[0]);
...
tabsContainer.addEventListener('click', (e) => {
    if (e.target.classList.contains('tab-btn')) {
        ...
        generateCards(e.target.dataset.category);
    }
});

// Mobile View: Toggle left column (navigation overlay)
const destinationTabs = document.querySelector('.mobile-tab-button');
const leftColumn = document.querySelector('.left-column');
const tabButtons = document.querySelectorAll('.tab-btn');
const optionLinks = document.querySelectorAll('.tab-btn + ul a'); // Select all links under tabs

if (window.innerWidth <= 768) {
// Toggle the left column visibility
destinationTabs.addEventListener('click', () => {
    if (leftColumn.style.left === '0px') {
        leftColumn.style.left = '-100%';
    } else {
        leftColumn.style.left = '0';
    }
});

// Close the left column when clicking a tab
tabButtons.forEach(tab => {
    tab.addEventListener('click', () => {
        leftColumn.style.left = '-100%'; // Close the overlay
        generateCards(tab.dataset.category);
    });
});

// Close the left column when clicking any option under the tab
...
// Close button inside the left column
document.querySelector('.left-column .close-btn').addEventListener('click', () => {
    leftColumn.style.left = '-100%';
});
}